...
export async function editListing(
  ...
  if (listingData.deletedPhotoPaths?.length > 0) {
    ...
    const photoUrls: string[] = listingData.deletedPhotoPaths.map(
      (path: string) =>
        `${process.env.SUPABASE_URL}/storage/v1/object/public/media/${path}`,
    );
  ...
  if (listingData.deletedFloorPlanPaths?.length > 0) {
    ...
    if (storageError) {
      ...
      throw new Error(
        "failed to delete floor plan files: " + storageError.message,
      );
    }

    // delete corresponding records from database
    const floorPlanUrls = listingData.deletedFloorPlanPaths.map(
      (path: string) =>
        `${process.env.SUPABASE_URL}/storage/v1/object/public/media/${path}`,
    );
    ...
    if (dbError) {
      ...
      throw new Error(
        "failed to delete floor plan records: " + dbError.message,
      );
    }
  }
